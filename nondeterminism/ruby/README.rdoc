...
Another option is to allocate a NonDeterminism::Generator to allow easier
cleanup after a computation is complete.  This can be done with

	NonDeterminism::Generator::new
...
Generators is a good way to make code confusion, however, and should be
avoided -- this capability is mainly provided to allow multi-threaded
programs to use nondeterminism from more than one thread (see below).
...
NonDeterminism::Generator object.  This can be done easily using thread
local variables:
