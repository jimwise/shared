...
type op = { doc : string; action : float list -> float list }

let dict = Hashtbl.create 32
let keys d = Hashtbl.fold (fun k v acc -> k :: acc) d []
let make_op name d a = Hashtbl.add dict name { doc = d; action = a }
let signal_error s = print_endline s
...
make_op "/" "replace top two values on the stack with their quotient" (function
  | y :: x :: stack -> (x /. y) :: stack
  | stack -> underflow stack)
;;
...
make_op "dup" "duplicate top value on the stack" (function
  | x :: stack -> x :: x :: stack
  | stack -> underflow stack)
;;

make_op "swap" "swap top two values on the stack" (function
  | y :: x :: stack -> x :: y :: stack
  | stack -> underflow stack)
;;
...
rpn []
(* XXX not sure why print_newline doesn't do it *)
